          ...
          <view class="moral-box u-shadow" v-for="(item,index) in moralList" :key="index">
            <view class="moral-info u-fx"> 
              <view class="u-fx-f1 u-fx-ver">
                <view class="moral-pur u-main-color u-font-1">{{ item.gradeName }} {{ item.beAssessedName }} {{ item.name }}</view>
...
export default {
  ...
  methods: {
    async eduListGet (tag = false) {
      ...
      const res = await actions.getEduList(req)
      if (tag) {
    ...
    detail (item) {
      this.$tools.navTo({
				url: `./detail?categoryDutyId=${item.id}&categoryId=${item.categoryId}&beAssessedCode=${item.beAssessedCode}`,
				title: '查看详情'
			})
    }
  }
