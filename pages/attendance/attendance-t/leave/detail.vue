<template>
	<view class="class-detail">
      ...
      <view class="u-padd">
		...
		<teacher-tree v-if="show" isRadio :teacherTag="teacherTag" v-show="teacherTag" :schoolInfo="schoolInfo" @close="teacherTag = false"
...
export default {
	...
	mounted() {
		this.show = this.$tools.getQuery().get("state") === '0' &&  this.$tools.getQuery().get("current") === '2'
		this.id = this.$tools.getQuery().get("id");
		this.getDetail()
	},
	methods: {
    async getDetail () {
      const res = await actions.getLeaveDetail(this.id)
