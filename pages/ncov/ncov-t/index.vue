	...
	methods: {
        ...
        thermometryDate: this.thermometryDate
			...
			if (res.data.length > 0) {
				this.startTime =this.$tools.getDateTime(res.data[0].startTime, 'second')
				this.endTime =this.$tools.getDateTime(res.data[0].endTime, 'second')
				this.showList()
			} else {
				this.dataCount = {
					reported: 0,
		...
		typeChange (index) {
			...
			this.startTime =this.$tools.getDateTime(data.startTime, 'second')
			this.endTime =this.$tools.getDateTime(data.endTime, 'second')
			this.showList()
		}
	}
};
