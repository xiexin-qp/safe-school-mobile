<template>
	<view class="u-page u-bg-fff">
		...
		<class-tree isCheck v-show="classTag" :classTag="classTag" :schoolInfo="schoolInfo" @close="classTag = false" @confirm="classSelcet" :classChecked="classList"></class-tree>
		<teacher-tree
			isCheck
			v-show="teacherTag"
...
export default {
	...
	methods: {
		// 表单回填
		async showData() {
			const res = await actions.getRelationData(this.id);
			this.controlList = res.data.deviceList.map(el => {
				return {
					...el,
					userId: el.id
				}
		...
		classSelcet(value) {
			console.log(value);
			this.classList = value.filter(item => item.type !== '1');
			this.classTitle = `已选择${this.classList.length}个班`;
			this.classTag = false;
		},
		teacherSelcet(value) {
			console.log(123, value);
			this.teacherList = value.filter(item => item.type === '2');
			this.teacherTitle = `已选择${this.teacherList.length}个教职工`;
		...
		submitForm() {
			...
			if (this.classList.length === 0 && this.teacherList.length === 0) {
