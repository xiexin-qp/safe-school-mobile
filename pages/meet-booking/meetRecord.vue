  ...
  <view class="u-page u-bg-fff u-auto">
    <view class="add" v-if="showTag === '1'">
      ...
      <view class="submit-box"
        ><view class="btn" @click="submit">提交</view></view
      >
    </view>
    <view class="edit" v-else>
      <view
        class="u-fx u-bd-b item-list"
        v-if="recordInfo.attachList.length > 0"
      >
        <view>图片：</view>
        <view class="u-fx-f1">
          <view class="u-fx-f1"
      ...
      <view class="u-fx item-list" v-if="recordInfo.otherList.length > 0">
        ...
        <view class="u-fx-f1">
          <view v-for="(item, i) in recordInfo.otherList" :key="i">
            {{ item.fileName }}
            <u-button
              class="button mar-l10"
              type="primary"
              @click="goDownload(item.fileUrl)"
              >查看</u-button
            >
          </view>
        </view>
