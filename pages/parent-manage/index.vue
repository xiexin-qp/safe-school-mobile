<template>
	<view class="parent-manage u-page u-fx-ver u-auto">
		<view class="share-dialog" v-if="isShare">
			<view class="share-btn" @tap="isShare = false"></view>
		</view>
		<u-popup mask-close-able mode="center" v-model="pShow" width="80%" height="auto" border-radius="16">
			...
			<view class="u-padd">
				<u-button type="default" @tap="pShow=false">关闭</u-button>
			</view>
		</u-popup>
...
export default {
	...
	methods: {
		...
		change (index) {
			...
			if (index === 0) {
				...
				this.dataList = this.sList.filter(item => {
					return this.pList.findIndex(it => {
		...
		share () {
			const _self = this
			const className = encodeURI(this.isBZR.gradeName + '' + this.isBZR.className)
			const linkUrl = `${config.host}/mobile-protal/parentReg?className=${className}&schoolYearId=${this.schoolYearId}&schoolCode=${this.userInfo.schoolCode}&gradeCode=${this.isBZR.gradeCode}&classCode=${this.isBZR.classCode}&date=${new Date().getTime()}#/`
