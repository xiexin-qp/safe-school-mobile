  ...
  <view class="schedule">
    <scroll-view  scroll-y="true" class="scroll-h">
      ...
      <view class="schedule-content u-fx-f1 u-fx-ver">
        <no-data msg="暂无数据~" v-if="(current === 0 && scheduleInfo.length === 0) || (current === 1 && scheduleList.dateSchedulings.length === 0 && scheduleList.timeSchedulings.length === 0)"></no-data>
        <view class="u-padd-l20 u-padd-r20" v-else>
          <view  v-if="current === 0">
            <view class="schedule-info u-padd-t20 u-padd-b20 u-padd-l30 u-padd-r30 u-mar-b30 u-fx-ver u-shadow" v-for="(item,index) in scheduleInfo" :key="index" @click="detail(0, item)">
              <view> 
              ...
              <view > 
                ...
                <u-tag  text="日常巡查" size="mini" shape="circle" v-if="item.taskTypes.indexOf('1') > -1"/>
                <u-tag class="u-mar-l20" text="设备巡查" type="warning"  size="mini" shape="circle" v-if="item.taskTypes.indexOf('2') > -1"/>
                <u-tag class="u-mar-l20" text="安全护导" type="success"  size="mini" shape="circle" v-if="item.taskTypes.indexOf('3') > -1"/>
              </view>
              <view> 
          ...
          <view v-else class="my-task">
            <view>
              ...
              <view class="task-info u-padd-r20 u-padd-l20 u-mar-b20 u-fx-jsb u-shadow" v-for="item in scheduleList.dateSchedulings" :key="item.taskId" @click="detail(1, item)">
                <view class="u-fx">
              ...
              <view class="task-type u-font-1" v-if="scheduleList.timeSchedulings && scheduleList.timeSchedulings.length !== 0">限时</view>
              <view class="task-info u-padd-r20 u-padd-l20 u-mar-b20 u-fx-jsb u-shadow" v-for="item in scheduleList.timeSchedulings" :key="item.taskId" @click="detail(1, item)">
                <view class="u-fx">
...
export default {
  ...
  mounted () {
    ...
    this.$refs.lxCalendar.nowMonth = this.$tools.getQuery().get("date").substring(5,6) === 0 ? this.$tools.getQuery().get("date").substring(6,7) : this.$tools.getQuery().get("date").substring(5,7)
