<template>
	<view>
		<scroll-view scroll-y="true" class="scroll-h">
			...
			<view v-if="currentKey === 1" class="">
				...
				<view v-if="classList.length > 0" class="u-fx-wp u-bd-b u-bg-fff u-padd-20 u-padd-t0">
					<u-tag class="u-mar-10" size="mini" type="primary" :text="item.gradeName + item.className" v-for="(item, i) in classList" :key="item.classCode" :index="i" />
				</view>
			</view>
		...
		<class-tree ref="classTree" isCheck v-show="classTag" :classTag="classTag" :schoolInfo="schoolInfo" @close="classClose" @confirm="classSelcet"></class-tree>
	</view>
</template>
