		...
		<scroll-view v-else scroll-y="true" class="scroll-y">
			...
			<view v-else class="u-padd-20">
				<u-table>
					...
					<u-tr v-for="(item, i) in statisticsList" :key="i">
						<u-td>{{ item.subject_name }}</u-td>
						<u-td>{{ item.total }}</u-td>
						<u-td>{{ item.unSubmit }}</u-td>
						<u-td>
							<view v-if="parseInt(item.unSubmit) !== 0" class="u-type-warning u-font-01" @click="goDetail(item.subject_code)">查看未交</view>
						</u-td>
					</u-tr>
				</u-table>
			</view>
