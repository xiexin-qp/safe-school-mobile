  ...
  <view class="statistics u-page">
    <view>
      <view class="search-list u-fx">
        <u-search placeholder="输入姓名搜索" v-model="keyword" :show-action="false" bg-color="#fff" shape="square" height="75"></u-search>
      ...
      <view class="search-box" v-if="isShow">
        <view class="search-info" v-for="(item,index) in studentList" :key="index"> 
          <view class="search-stu u-bd-b" @click="choose(item)"> 
            {{ item.userName }} , {{ item.workNo }} , {{ item.gradeName }} / {{ item.className }}
          </view>
        </view>
      </view>
      <view class="student-box u-fx-ac">
        ...
        <view class="u-fx-ac" v-else> 
          ...
          <view>  {{ studentInfo.userName }} {{ studentInfo.workNo }} {{ studentInfo.gradeName }} {{ studentInfo.className }} </view>
        </view>
      </view>
  ...
  methods: {
    ...
    async detail (item) {
      if (JSON.stringify(this.studentInfo) == '{}') {
        ...
        this.$tools.navTo({
          url: `./add?id=${item.id}&name=${this.studentInfo.userName}&code=${this.studentInfo.userCode}&classCode=${this.studentInfo.classCode}&workNo=${this.studentInfo.workNo}&schoolYearId=${this.studentInfo.schoolYearId}`,
          title: '评分'
        })
      }
    },
