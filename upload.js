...
let buildModule = "云平台移动端";
if (system === 'protal') {
	buildModule = '主模块'
} else if (system === 'protal-card') {
	buildModule = '电子班牌模块'
} else if (system === 'protal-safe') {
	buildModule = '安防系统模块'
}
const envHost = {
  prod: {
...
inquirer.prompt(promptList).then(answers => {
	...
	zip.writeZip(`unpackage/dist/build/${system}.zip`);
	var formData = {
	  file: fs.createReadStream(
	    path.resolve(__dirname, `unpackage/dist/build/${system}.zip`)
	  ),
	};
	request.post(
	  {
